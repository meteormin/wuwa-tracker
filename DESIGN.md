...
- `crates/wuwa-tracker-app`: `wuwa-tracker` GUI-compatible binary, `wuwa-tracker-cli` CLI/server binary, application service layer를 제공합니다. Tauri GUI, Axum HTTP server, CLI subcommand를 같은 app service 위에서 실행합니다.
...
- API server: `make serve` 또는 `cargo run -p wuwa-tracker --bin wuwa-tracker-cli -- serve --host 127.0.0.1 --port 3000`
- Fullstack server: `make serve WEBUI=1` 또는 `cargo run -p wuwa-tracker --bin wuwa-tracker-cli -- serve --webui`
- CLI: `cargo run -p wuwa-tracker --bin wuwa-tracker-cli -- <command> [args]`
