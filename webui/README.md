...
- `src/lib/api/index.ts`: Tauri invoke와 HTTP API 호출 wrapper
- `src/lib/types.ts`: Rust API 응답과 맞춘 프론트 타입
...
UI 번역 원본은 프론트엔드 폴더가 아니라 Rust core에서 embed하는 `locales/ui/*.json`에 있습니다. 새 문구를 추가할 때는 embed 리소스의 JSON key를 추가하고, 프론트에서는 `$t("key")` 형태로 참조합니다.
