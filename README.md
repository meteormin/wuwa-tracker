...
- `crates/wuwa-tracker-app`: 실행 파일, Tauri GUI, Axum 기반 API-only `serve` 서버, CLI subcommand
- `webui`: Svelte + Vite UI. Tauri GUI와 Vite 개발 서버에서 사용합니다.
...
기본 주소는 `http://127.0.0.1:3000`입니다. `serve`는 WebUI static asset을 제공하지 않고 `/api/*` route만 제공합니다.
